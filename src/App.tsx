...
const signLabelById = Object.fromEntries(signOptions.map((item) => [item.id, item.label])) as Record<SignId, string>;
    ...
    <section className="panel two-column">
      ...
      <div className="stack">
        ...
        <div className="history-summary">
          <strong>保存済みデータ</strong>
          <span>合計 {history.length} 件</span>
        </div>
        {history.length === 0 ? (
          <Empty text="保存済みのサインはまだありません。" />
        ) : (
          history.map((item) => (
            <article className="history-card sign-history-card" key={item.id}>
              <div className="history-card-head">
                ...
                <button className="icon-button danger" type="button" onClick={() => setHistory((current) => current.filter((historyItem) => historyItem.id !== item.id))} aria-label="この保存データを削除">
                  <Trash2 size={16} />
                </button>
              </div>
              {item.checked.length > 0 ? (
                <ul className="sign-chip-list" aria-label="チェックされた項目">
                  {item.checked.map((signId) => (
                    <li key={signId}>{signLabelById[signId]}</li>
                  ))}
                </ul>
              ) : (
                <p className="history-line">チェック項目なし</p>
              )}
              {item.note ? (
